  ...
  return (
    <div className="fixed inset-0 z-[1100] flex flex-col bg-slate-50 p-4">
      ...
      <div className="mt-3 flex flex-col gap-3 overflow-y-auto pb-4">
        {selected === null ? (
          <section className="rounded-lg bg-white py-1 shadow-sm">
            {matches.map((l) => (
              <button
                key={l}
                type="button"
                onClick={() => setQuery(l)}
                className="block w-full px-4 py-2.5 text-left text-sm font-semibold text-slate-900"
              >
                {l}
              </button>
            ))}
            {matches.length === 0 && (
              <p className="p-4 text-center text-sm text-slate-400">일치하는 장소가 없습니다</p>
            )}
          </section>
        ) : (
          <>
            <section className="rounded-lg bg-white p-3 shadow-sm">
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-slate-900">{selected}</span>
            ...
            <section className="rounded-lg bg-white p-3 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900">최근 방문</h3>
              <p className="pb-1 pt-0.5 text-xs text-slate-400">
