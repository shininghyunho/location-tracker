    ...
    >
      <div className="flex items-center justify-between pb-2">
        ...
        <button
          ...
          className="px-3 py-1 text-slate-500 disabled:text-slate-300 dark:text-slate-400 dark:disabled:text-slate-600"
        >
          ▶
        </button>
      </div>
      <div className="grid grid-cols-7 gap-1 pb-1 text-center text-[10px] text-slate-400 dark:text-slate-500">
        {WEEKDAYS.map((w) => (
          <div key={w}>{w}</div>
        ))}
      </div>
