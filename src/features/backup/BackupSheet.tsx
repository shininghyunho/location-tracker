...
export function BackupSheet({ onClose }: BackupSheetProps) {
  ...
  return (
    <BottomSheet onClose={onClose}>
      <h2 className="text-lg font-bold text-slate-900">데이터 백업</h2>
      <p className="mt-1 text-sm text-slate-500">위치와 체류 기록 전체를 파일 하나로 내보내요.</p>
      <dl className="mt-3 space-y-1.5 text-sm">
        <div className="flex justify-between">
          <dt className="text-slate-500">보관 중인 기록</dt>
          <dd className="text-slate-700">
            위치 {pointCount.toLocaleString()}건 · 체류 {stayCount.toLocaleString()}건
          </dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-slate-500">마지막 내보내기</dt>
          <dd className="text-slate-700">{lastExportAt ? fmtLastExport(lastExportAt) : '없음'}</dd>
        </div>
      </dl>
      <button
        type="button"
        ...
        disabled={exporting}
        className="mt-5 w-full rounded-lg bg-blue-600 py-2.5 text-sm font-semibold text-white disabled:bg-slate-300"
      >
