...
1. [최신 릴리스](https://github.com/shininghyunho/location-tracker/releases/latest)에서 `.apk` 파일을 폰으로 내려받는다. → [v0.2.0-beta 직접 다운로드](https://github.com/shininghyunho/location-tracker/releases/download/v0.2.0-beta/location-tracker-v0.2.0-beta.apk)
...
- **삼성 갤럭시**는 한 번만 추가로: 설정 → 배터리 → 백그라운드 사용 제한 → **절대 잠자지 않는 앱**에 위치트래커를 등록한다. 삼성 절전이 앱을 재우고 죽이는 것을 막아 수집 공백이 줄어든다.
- 상태바에 남는 수집 알림 아이콘은 코드로 무음(LOW)까지 내렸지만 Android가 포그라운드 서비스 알림의 그 이하 하향을 막아 완전히 없앨 수는 없다. 직접 없애려면 **(권장)** 설정 → 알림 → 위치트래커 → "BackgroundGeolocation" 채널을 꺼두면 영구적으로 사라지고 수집에는 영향이 없다. 급하면 알림을 옆으로 스와이프해 지워도 되지만(Android 13+는 서비스가 유지된 채 알림만 사라짐) 재부팅하면 다시 나타난다.
